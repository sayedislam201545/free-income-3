...
let submissionsContent = fs.readFileSync('fix_submissions.cjs', 'utf8').split('`')[3] || fs.readFileSync('fix_submissions.cjs', 'utf8').split('`')[1];
...
let reqsContent = fs.readFileSync('reqs_clean.txt', 'utf8');
let adminSettingsContent = fs.readFileSync('fix_admin_settings_now.cjs', 'utf8').split('`')[1];
let adminVipContent = fs.readFileSync('fix_admin_vip.cjs', 'utf8').split('`')[1];
...
function AdsRewardsEditor({ onClose, onSave, initialValues }: any) {
  const [values, setValues] = useState(initialValues || { normal: {}, vip: {} });
  return (
    <div className="space-y-6">
      <div className="flex justify-between">
...
function FeatureTogglesEditor({ onClose, onSave, initialValues }: any) {
  const [values, setValues] = useState(initialValues || { maintenance: false });
  return (
    <div className="space-y-6">
      <div className="flex justify-between">
