...
const regex = /function AdminPayments\(\) \{[\s\S]*?return \([\s\S]*?<div className="space-y-6">[\s\S]*?<div className="flex justify-between items-center">[\s\S]*?<h2 className="text-xl font-bold">Payments & Requests<\/h2>[\s\S]*?<\/div>[\s\S]*?<div className="flex space-x-2 bg-\[#1C2331\] p-1\.5 rounded-xl overflow-x-auto w-full mb-6 no-scrollbar">[\s\S]*?<\/div>[\s\S]*?\{activeType === "requests" \? \([\s\S]*?<AdminRequests \/>[\s\S]*?\) : \([\s\S]*?<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?\)[\s\S]*?\}[\s\S]*?\}/;
