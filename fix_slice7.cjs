...
const badStr = ` from "react";import { Routes`;
const badIndex = code.lastIndexOf(badStr);
if (badIndex !== -1 && badIndex > 100) {
    code = code.substring(0, badIndex); // Trim it out completely
}

// Check if we need to append FeatureTogglesEditor by seeing if it's already there
